...
async function addLogoToPDF(doc: jsPDF, x: number, y: number, width: number, height: number) {
  try {
    ...
    const logoResponse = await fetch(
      "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Deer%20Stats%20Logo-vg8dkQ8F0fmtSQkZpS0rkWfqakC3Ua.png",
    )
...
async function generatePDF(results: any[]) {
  ...
  // Add logo
  await addLogoToPDF(doc, margins, 5, 20, 20)

  // Add title with branding
  doc.setFontSize(20)
  doc.setTextColor(52, 73, 94) // Dark blue-gray color
  doc.text("Deer Stats - Weather Data Analysis Report", margins + 25, 15)

  // Add generation date and info
  doc.setFontSize(10)
  doc.setTextColor(100, 100, 100) // Gray color
  doc.text(`Generated on: ${new Date().toLocaleString()}`, margins + 25, 22)
  doc.text(`Total Images Analyzed: ${results.length}`, margins + 25, 27)
  ...
  autoTable(doc, {
    ...
    body: tableData,
    startY: 35,
    margin: { top: 35, left: margins, right: margins, bottom: 25 },
    styles: {
      fontSize: 8,
      cellPadding: 2,
      overflow: "linebreak",
    ...
    didDrawPage: (data) => {
      ...
      doc.setFontSize(8)
      doc.setTextColor(100, 100, 100)
      doc.text("Visit us at: weather-analysis.deerstats.com", margins, pageHeight - 4)
    },
  })
